...
$(AS_CPU_PATH)/VcFntDat/arial.ccf:$(AS_CPU_PATH)/VcFntDat/arial.vco
	 $(LINK) '$^' -o '$@' -warningLevel2 -m arial.ttf -name Visu -profile 'False' -vcr 4734 -sfas
...
# Font calibrib
#
ifneq ($(VC_FONT_calibrib),1)
VC_FONT_calibrib=1
$(AS_CPU_PATH)/VcFntDat/calibrib.vco:$(AS_CPU_PATH)/VcFntDat/calibrib.vci
	 $(VCC) -f '$<' -o '$@' $(TTFFLAGS_Visu) $(VCCFLAGS_Visu) -sfas

$(AS_CPU_PATH)/VcFntDat/calibrib.ccf:$(AS_CPU_PATH)/VcFntDat/calibrib.vco
	 $(LINK) '$^' -o '$@' -warningLevel2 -m calibrib.ttf -name Visu -profile 'False' -vcr 4734 -sfas

$(AS_CPU_PATH)/calibrib.br:$(AS_CPU_PATH)/VcFntDat/calibrib.ccf
	 $(MODGEN) -so $(VC_STATIC_OPTIONS_Shared) -fw '$(VCFIRMWAREPATH)' -m $(VCLOD) -f '$<' -o '$@' -d vcgclass -v V1.00.0 -profile False -vc '$(VCOBJECT_Visu)' -b
endif
...
$(AS_CPU_PATH)/VcFntDat/arialbd.ccf:$(AS_CPU_PATH)/VcFntDat/arialbd.vco
	 $(LINK) '$^' -o '$@' -warningLevel2 -m arialbd.ttf -name Visu -profile 'False' -vcr 4734 -sfas

$(AS_CPU_PATH)/arialbd.br:$(AS_CPU_PATH)/VcFntDat/arialbd.ccf
	 $(MODGEN) -so $(VC_STATIC_OPTIONS_Shared) -fw '$(VCFIRMWAREPATH)' -m $(VCLOD) -f '$<' -o '$@' -d vcgclass -v V1.00.0 -profile False -vc '$(VCOBJECT_Visu)' -b
endif

FONT_MODULES_Visu=$(TEMP_PATH_ROOT_Visu)/Objects/$(AS_CONFIGURATION)/$(AS_TEMP_PLC)/arial.br $(TEMP_PATH_ROOT_Visu)/Objects/$(AS_CONFIGURATION)/$(AS_TEMP_PLC)/calibrib.br $(TEMP_PATH_ROOT_Visu)/Objects/$(AS_CONFIGURATION)/$(AS_TEMP_PLC)/arialbd.br 
